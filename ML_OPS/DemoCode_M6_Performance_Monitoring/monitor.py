    ...
    if drift_summary is None:
        try:
            ...
            if hasattr(report, 'as_dict'):
                ...
                print(f"✅ Got report as dict: {type(report_dict)}")
                ...
                # Find the DataDriftPreset entry
                for metric_data in report_dict.get("metrics", []):
                    metric_name = str(metric_data.get("metric", ""))
                    if "DataDriftPreset" in metric_name:
            ...
            # Try json() method
                ...
                #             drift_summary = create_drift_summary_from_result(metric_data['result'])
                #             break
                for metric_data in result_dict.get("metrics", []):
                    metric_name = str(metric_data.get("metric", ""))
                    if "DataDriftPreset" in metric_name:
                        result = metric_data.get("result", {})
                        drift_summary = create_drift_summary_from_result(result)
